...
@app.post("/api/upload/", response_model=schemas.File)
def upload_file(
...
@app.post("/api/create-admin/", response_model=schemas.User)
...
@app.post("/api/create_file_tag/", response_model=schemas.FileTagCreate)
...
@app.get("/api/file_tags/", response_model=List[schemas.Tags])
def read_file_tags(db: Session = Depends(get_db)):
    return crud_file.get_file_tags(db)

@app.delete("/api/delete_file_tag/")
